...
describe('resolvePlaceholders', () => {
  it('always supplies name and event without a mapping', () => {
    ...
    expect(values).toEqual({ name: 'Rafan Ahamad Sheik', event: 'IoT Workshop 2026' })
  })
  ...
  it('prints an officer-set value for the person it was set against', () => {
    const values = resolvePlaceholders(
  ...
  it('prefers the officer-set value over the default', () => {
    const values = resolvePlaceholders(
      form([{ key: 'Place', source: 'perPerson', defaultValue: 'Participant' }]),
      submission({}, [{ key: 'Place', value: '1st' }]),
      'Someone',
