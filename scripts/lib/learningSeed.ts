...
export function insertAfterCaption<T, U>(
  ...
  const index = blocks.findIndex(
    (block) =>
      (block as { caption?: string })?.caption?.includes(caption) ?? false,
  );
  if (index === -1)
    throw new Error(`no block with caption containing "${caption}"`);
  return [
...
export async function simulatorId(
  payload: Payload,
  slug: string,
): Promise<number> {
...
export const accordionBlock = (
  items: ReturnType<typeof accordionItem>[],
  heading?: string,
) => ({
 ...
 */
export async function ensurePlaceholderMedia(
  payload: Payload,
): Promise<number> {
  const found = await payload.find({
    collection: "media",
  ...
  const data = readFileSync(
    path.resolve(process.cwd(), "public/placeholder/placeholder.jpg"),
  );
  const media = await payload.create({
    collection: "media",
 ...
 * state a member can change from the admin, and a stale number silently tags a
