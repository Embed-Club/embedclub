...
function imageRowsWithoutPicture(
  steps: StepRow[] | null | undefined,
): string[] {
      ...
      ({ data }) => {
        ...
        if (data?.sectionOf) {
          if (!data?.sectionLabel?.trim()) {
            throw new APIError(
              "A section needs a label - e.g. A Section, or Day 1.",
              400,
            );
          }
        ...
        if (data?.showCertificate) {
          ...
          if (names !== 1) {
            problems.push(
              `exactly one question marked as the person's name (found ${names})`,
            );
          }
          if (emails !== 1) {
            problems.push(
              `exactly one question marked as the person's email (found ${emails})`,
            );
          }
          if (problems.length > 0) {
            throw new APIError(
              `Certificates need ${problems.join(", and ")}.`,
              400,
            );
          }
        }
