...
const SortOrderSelectCategory: FieldClientComponent = ({ path }) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null)
      try {
        const res = await fetch('/api/member-categories?limit=1000&sort=sortOrder', {
          credentials: 'include',
        })
...
export default SortOrderSelectCategory
