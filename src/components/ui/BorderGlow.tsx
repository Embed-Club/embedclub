...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  useEffect(() => {
    ...
    const handleGlobalPointerMove = (e: MouseEvent) => {
      ...
      if (isInside) {
        ...
        const outerProximity = Math.max(
          0,
          1 - (dist - Math.min(rect.width, rect.height) / 2) / nearbyRange,
        )
  ...
  const buildMeshGradients = (colors: string[]) => {
    ...
    const gradients = POS.map(
      (p, i) =>
        `radial-gradient(at ${p}, ${colors[Math.min(MAP[i], colors.length - 1)]} 0px, transparent 50%)`,
    )
