    ...
    (opening: boolean) => {
      ...
      if (changeMenuColorOnOpen) {
        const targetColor = opening 
          ? (mounted && isDark ? '#ffffff' : '#000000')
          : (mounted && isDark ? '#e9e9ef' : '#111111');
        colorTweenRef.current = gsap.to(btn, { color: targetColor, delay: 0.18, duration: 0.3, ease: 'power2.out' });
      } else {
        gsap.set(btn, { color: mounted && isDark ? '#e9e9ef' : '#111111' });
      }
    },
    [mounted, isDark, changeMenuColorOnOpen]
  );
    ...
    >
      <div
        ...
        style={{
          ...
          ['--sm-bg' as any]: mounted && isDark ? '#0b0b0f' : '#ffffff',
          ['--sm-fg' as any]: mounted && isDark ? '#f5f5f5' : '#000000',
          ['--sm-muted' as any]: mounted && isDark ? '#c2c2c2' : '#111111'
        }}
      ...
      >
        ...
        <aside
          ...
          style={{ WebkitBackdropFilter: 'blur(12px)', background: 'var(--sm-bg,#fff)', color: 'var(--sm-fg,#000)' }}
          aria-hidden={!open}
        >
...
.sm-scope .sm-panel-item { position: relative; color: var(--sm-fg,#000000); font-weight: 600; font-size: clamp(2.2rem, 6vw, 3.4rem); cursor: pointer; line-height: 1; letter-spacing: -1px; text-transform: uppercase; transition: background 0.25s, color 0.25s; display: inline-block; text-decoration: none; padding-right: 1.4em; }
