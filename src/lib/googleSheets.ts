 ...
 * opaque JSON blob.
 *
 * Payload remains the source of truth; this is a convenience for members who
...
async function readHeaders(sheetId: string): Promise<string[]> {
  ...
  if (!res.ok) {
    throw new Error(
      `Reading sheet headers failed (${res.status}): ${await res.text()}`,
    );
  }
...
async function writeHeaders(sheetId: string, headers: string[]): Promise<void> {
  ...
  if (!res.ok) {
    throw new Error(
      `Writing sheet headers failed (${res.status}): ${await res.text()}`,
    );
  }
}
...
async function readRowIndex(
  ...
  if (!res.ok) {
    throw new Error(
      `Reading sheet ids failed (${res.status}): ${await res.text()}`,
    );
  }
...
async function writeRow(
  ...
  if (!res.ok) {
    throw new Error(
      `Sheets row update failed (${res.status}): ${await res.text()}`,
    );
  }
}
...
function questionLabels(form: Form): string[] {
  ...
  for (const step of form.steps ?? []) {
    for (const field of step.fields ?? []) {
      ...
      if (field.label && !labels.includes(field.label))
...
function uploadLabels(form: Form): Set<string> {
  ...
  for (const step of form.steps ?? []) {
    for (const field of step.fields ?? []) {
      if (field.fieldType === "imageUpload" && field.label)
...
function cellValue(answer: unknown, isUpload = false): string {
  ...
  // A bare Drive id is useless in a spreadsheet; members want the link.
  if (isUpload && typeof answer === "string" && answer !== "")
    return driveViewUrl(answer);
 ...
 * untrustworthy.
 */
async function reconcileHeaders(
  sheetId: string,
  wanted: string[],
): Promise<string[]> {
...
export async function syncPendingSubmissions(
  ...
  if (!sheetsCredentialsPresent()) {
    return {
      synced: 0,
      failed: 0,
      skipped: "Google service account is not configured",
    };
  }
  ...
  const byForm = new Map<
    number,
    { form: Form; submissions: FormSubmission[] }
  >();
  ...
  for (const { form, submissions } of byForm.values()) {
    ...
    try {
      headers = await reconcileHeaders(sheetId, [
        ...BASE_HEADERS,
        ...questionLabels(form),
      ]);
      ...
      console.error(
        `[Sheets] Header setup failed for form "${form.slug}":`,
        error,
      );
    ...
    for (const submission of submissions) {
      try {
        const answers = (submission.answersByLabel ?? {}) as Record<
          string,
          unknown
        >;
        ...
        console.error(
          `[Sheets] Sync failed for submission ${submission.id}:`,
          error,
        );
        failed += 1;
      }
